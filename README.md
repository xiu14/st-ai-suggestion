...
一个为 SillyTavern / JS-Slash-Runner 设计的 AI 回复建议脚本，帮助你快速生成符合角色设定的回复选项。

## ✨ 功能特性

- **智能回复建议** - 基于对话上下文和角色设定，自动生成多个风格各异的回复建议
- **一键填入/发送** - 点击建议按钮后可选择"填入并编辑"或"直接发送"
- **大纲生成** - 通过独立按钮输入剧情大纲，让 AI 根据大纲生成定向建议
...
### 推荐：JS-Slash-Runner JSON 版

1. 打开 SillyTavern，并确认已安装/启用 JS-Slash-Runner
2. 进入 JS-Slash-Runner 的脚本管理界面
3. 导入本仓库中的 `ai-suggestion-js-slash.json`
4. 启用脚本后，会出现 **立即生成** 和 **写大纲** 两个按钮

> JSON 版是独立版本，不需要再通过 SillyTavern 扩展安装本仓库。若同时安装扩展版和 JSON 版，建议只启用其中一个，避免重复监听。
...
1. 安装并启用 JSON 脚本后，会出现 **立即生成** 和 **写大纲** 两个按钮
2. **点击 立即生成** → 自动分析对话上下文，生成回复建议
